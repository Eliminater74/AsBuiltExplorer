...
namespace AsBuiltExplorer
{
    public class VINDecoder
    {
        ...
        // Position 1-3: WMI
        static readonly Dictionary<string, (string Make, string Type, string Country)> WMIData = new Dictionary<string, (string, string, string)>
        {
            { "1FA", ("Ford", "Car", "USA") },
        ...
        static readonly Dictionary<char, string> RestraintCodes = new Dictionary<char, string>
        {
        ...
        static readonly Dictionary<char, string> GVWRCodes = new Dictionary<char, string>
        {
        ...
        static readonly Dictionary<string, (string Model, string Trim, string Drive)> ModernSeries = new Dictionary<string, (string, string, string)>
        {
            // Mustang S550
        ...
        static readonly Dictionary<char, string> LegacyBodyCodes = new Dictionary<char, string>
        {
        ...
        static readonly Dictionary<string, string> LegacySeriesCodes = new Dictionary<string, string>
        {
            { "15", "XLT 2WD" },
            { "16", "XLT 4WD" },
        ...
        // Position 8: Engine
        static readonly Dictionary<char, string> EngineCodes = new Dictionary<char, string>
        {
            { '5', "5.4L 3V Triton V8" },
            { 'W', "4.6L V8 (Romeo)" },
            { '6', "4.6L V8 (Windsor)" },
        ...
        static readonly Dictionary<char, string> YearCodes = new Dictionary<char, string>
        {
        ...
        // Position 11: Plant
        static readonly Dictionary<char, string> PlantCodes = new Dictionary<char, string>
        {
            { 'F', "Dearborn, MI" },
        ...
        public static List<DecodeResult> Decode(string vin)
        {
            ...
            var c4 = vin[3];
            var m4 = "Unknown";
            var isTruck = type.Contains("Truck") || type.Contains("SUV");
            var isCar = type.Contains("Car");
            ...
            {
                var c5 = vin[4]; // Body
                var s67 = vin.Substring(5, 2); // Series/Trim

                var bodyStr = LegacyBodyCodes.ContainsKey(c5) ? LegacyBodyCodes[c5] : "Unknown Body";
                var trimStr = LegacySeriesCodes.ContainsKey(s67) ? LegacySeriesCodes[s67] : "Unknown Series";
            ...
            // --- 8: Engine ---
            var c8 = vin[7];
            var engineStr = EngineCodes.ContainsKey(c8) ? EngineCodes[c8] : "Unknown Engine";
            results.Add(new DecodeResult { Position = "8", Value = c8.ToString(), Meaning = "Engine", Notes = engineStr });

            // --- 9: Check Digit Verification ---
            var checkDigit = vin[8];
            var isValid = VerifyCheckDigit(vin);
            ...
            if (int.TryParse(GetYear(c10), out modelYear))
            {
                if (modelYear < 2010)
                {
                    ...
                    var seqStr = vin.Substring(11);
                    var digits = Regex.Replace(seqStr, "[^0-9]", "");
                    ...
                    if (int.TryParse(digits, out int seqNum))
                    {
                        ...
                        var monthsAfterAug = (double)seqNum / 8500.0;
                        var startProduction = new DateTime(modelYear - 1, 8, 1);
                        var estDate = startProduction.AddMonths((int)monthsAfterAug);
