...
internal sealed class modAsBuilt
{
  ...
  const int GENERIC_WRITE = 1073741824 /*0x40000000*/;
  const int CREATE_ALWAYS = 2;
  const int OPEN_EXISTING = 3;
  const int FILE_SHARE_READ = 1;
  const int FILE_SHARE_WRITE = 2;
  const int FILE_FLAG_OVERLAPPED = 1073741824 /*0x40000000*/;
  const int FILE_FLAG_NO_BUFFERING = 536870912 /*0x20000000*/;
  const int FILE_ATTRIBUTE_NORMAL = 128 /*0x80*/;
  const int SECURITY_ATTRIBUTES_UDTsize = 12;
  const int MEDIASERIALNUMDATA_UDTsize = 16 /*0x10*/;
  const int INVALID_HANDLE_VALUE = -1;
  const uint FSCTL_LOCK_VOLUME = 589848;
  const uint FSCTL_DISMOUNT_VOLUME = 589856 /*0x090020*/;
  const uint IOCTL_STORAGE_MEDIA_REMOVAL = 2967556;
  const uint IOCTL_STORAGE_EJECT_MEDIA = 2967560;
  const uint IOCTL_STORAGE_LOAD_MEDIA = 2967564;
  const uint IOCTL_SFFDISK_DEVICE_COMMAND = 499332;
  const uint IOCTL_STORAGE_GET_MEDIA_SERIAL_NUMBER = 15532032 /*0xED0000*/;
  const uint SFFDISK_DC_GET_VERSION = 0;
  const uint SFFDISK_DC_LOCK_CHANNEL = 1;
  const uint SFFDISK_DC_UNLOCK_CHANNEL = 2;
  const uint SFFDISK_DC_DEVICE_COMMAND = 3;
  const uint SFFDISK_SC_GET_CARD_VERSION = 10;
  ...
  static extern void CopyMemory_FromBYTEtoINT32(
    ref int lpvDest,
    ref byte lpvSource,
    uint cbCopy);

  [DllImport("kernel32", EntryPoint = "CreateFileA", CharSet = CharSet.Ansi, SetLastError = true)]
  static extern IntPtr CreateFile_NoUDT(
  ...
  static extern int WriteFile_NoUDT(
    IntPtr hFile,
    ref byte lpBuffer,
  ...
  static extern int ReadFile_NoUDT(
    IntPtr hFile,
    ref byte lpBuffer,
  ...
  static extern int SetEndOfFile(IntPtr hFile);

  [DllImport("kernel32", CharSet = CharSet.Ansi, SetLastError = true)]
  static extern int SetFilePointerEx(
    IntPtr hFile,
  ...
  static extern bool GetFileSizeEx([In] IntPtr hFile, [In, Out] ref long lpFileSize);
  ...
  static extern bool DeviceIoControl_ByteBuffers(
    IntPtr hDevice,
    uint dwIoControlCode,
    ref byte lpInBuffer,
    uint nInBufferSize,
    ...
    ref int retModuleCount)
  {
    ...
    retModuleCount = 0;
    var num1 = checked (inpFileCount - 1);
    var index1 = 0;
    bool flag;
    while (index1 <= num1)
    {
      var path = inpFileArray[index1];
      var strArray1 = new string[1];
...
label_15:
    ...
    ref string retShortName)
  {
    ...
    retShortName = "";
    retIdx = -1;
    var num = checked (modlistCount - 1);
    var index = 0;
    while (index <= num)
    {
    ...
    ref int retModuleCount)
  {
    ...
    var num = checked (strArray2.Length - 1);
    var index = 0;
    while (index <= num)
    {
...
label_9:
  ...
  static string AsBuilt_LoadFile_AB_HTML_GetText(string inpStr, int pos)
  {
    if (pos < 1)
      return "";
...
label_6:
    ...
    ref string retCCCdata)
  {
    ...
    var str1 = "";
    var str2 = "";
    var xmlTextReader = new XmlTextReader(inpFileName);
    var str3 = "";
    var flag1 = false;
    var flag2 = false;
    var flag3 = false;
          ...
          while (xmlTextReader.NodeType == XmlNodeType.Element)
          {
            ...
            switch (name)
            {
              ...
              case "DATA":
                if (flag1)
                {
                  var attribute = xmlTextReader.GetAttribute("LABEL");
                  checked { ++index; }
                  retModuleAddresses = (string[]) Utils.CopyArray((Array) retModuleAddresses, (Array) new string[checked (index + 1)]);
                  retModuleDatas = (string[]) Utils.CopyArray((Array) retModuleDatas, (Array) new string[checked (index + 1)]);
              ...
              case "NODEID":
                ...
                goto label_1;
              case "VIN":
                var str4 = xmlTextReader.ReadInnerXml();
                retVIN = str4;
...
label_12:
  ...
  public static string AsBuilt_Dec2Bin(ulong num)
  {
    ...
    do
    {
      var d2 = decimal.Remainder(new decimal(num), 2M);
    ...
    string sDefaultPath)
  {
    ...
    var str = "";
    var num1 = checked (sFileTypeArray.Length - 1);
    var index1 = 0;
    while (index1 <= num1)
    {
    ...
    var dialogResult = openFileDialog.ShowDialog((IWin32Window) ownerForm);
    var flag = false;
    if (dialogResult != DialogResult.Cancel)
    {
      ...
      var num4 = checked (retSelFileCount - 1);
      var index2 = 0;
      while (index2 <= num4)
      {
  ...
  {
    var Expression = (HtmlElement) null;
    var tickCount = modAsBuilt.System_GetTickCount();
    do
    {
  ...
  public static string StorageMedia_GetSerialNumber(string drvLetter, ref string retError)
  {
    ...
    var fileNoUdt = modAsBuilt.CreateFile_NoUDT(ref lpFileName, -1073741824 /*0xC0000000*/, 3, IntPtr.Zero, 3, 0, 0);
    ...
    var udtBytes2 = new byte[1];
    modAsBuilt.StorageMedia_GetSerialNumber_BuildCommand(ref udtBytes2);
    var numArray = new byte[1001];
    uint lpBytesReturned = 0;
    var flag = modAsBuilt.DeviceIoControl_ByteBuffers(fileNoUdt, 499332U, ref udtBytes2[0], checked ((uint) udtBytes2.Length), ref udtBytes2[0], checked ((uint) udtBytes2.Length), ref lpBytesReturned, IntPtr.Zero);
    var str = "";
    if (!flag)
    {
      retError = new Win32Exception().Message;
    ...
    {
      var num1 = checked (udtBytes2.Length - 17);
      var num2 = checked (udtBytes2.Length - 1);
      var index = num1;
      while (index <= num2)
      {
  ...
  static void StorageMedia_GetSerialNumber_BuildCommand(ref byte[] udtBytes)
  {
    var num1 = 51;
    ...
    bool NoDupesAllowed)
  {
      ...
      var index = 0;
      var low = 0;
      var high = dstItemCount - 1;
      while (low <= high)
      {
          var mid = (low + high) / 2;
          var cmp = string.Compare(dstArray[mid], strItemToAdd, StringComparison.Ordinal);
          if (cmp < 0) low = mid + 1;
  ...
  static void ArraySsorted_RemoveItem(
    ref string[] strArray,
    ...
    CompareMethod compMethod = CompareMethod.Binary)
  {
    ...
    static int ArraySsorted_FindItemPos(
    ref string[] strArray,
    int strArrayItemCount,
    string strToFindPos)
  {
      var low = 0;
      var high = strArrayItemCount - 1;
      while (low <= high)
      {
          var mid = (low + high) / 2;
          var cmp = string.Compare(strArray[mid], strToFindPos, StringComparison.Ordinal);
          if (cmp < 0) low = mid + 1;
          else if (cmp > 0) high = mid - 1;
          else return mid;
      }
    ...
    static void ArrayS_ShiftItemsRight(
    ref string[] strArray,
    ...
    string defaultSaveName)
  {
    ...
    var dialogResult = saveFileDialog.ShowDialog((IWin32Window) ownerForm);
    var str = "";
    if (dialogResult != DialogResult.Cancel)
